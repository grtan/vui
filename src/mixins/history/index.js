...
let action // 操作：新建、前进、后退、刷新还是replace
...
export const mixin = {
  ...
  watch: {
    $route () {
      if (!action) { // 还未判断出操作是啥（有可能多个组件引入该mixin，只会判断一次）
        switch (true) {
          case !window.history.state || !window.history.state[name]: // 新建历史记录，或者replace（history.state会被清除）
            ...
            window.history.replaceState(Object.assign({}, window.history.state || {}, {
              [name]: isReplace ? previousTime : Date.now()
            }), '')
  ...
  created () {
    if (!replace && this.$router) {
      ...
      this.$router.replace = function () {
        isReplace = true
        previousTime = (window.history.state && window.history.state[name]) || Date.now()
        replace(...arguments)
      }
    }
  }
