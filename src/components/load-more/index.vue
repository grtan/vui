<template>
  <div
    class="vui-load-more"
    :vui-position="position"
    :vui-pull="pull"
    :vui-loading="status==='loading'"
    :vui-complete="['init','loading'].indexOf(status)===-1"
  ...
  >
    <div
      class="vui-load-more-loading"
      :style="{[this.sizeProp]:`${this.size}px`}"
      ...
      <div class="vui-load-more-loading-box" ref="box">
        <slot name="loading">
          <div class="vui-load-more-default">
            <div
              class="vui-load-more-icon"
              :style="iconStyle"
              ref="icon"
              v-show="['init','loading'].indexOf(status)!==-1"
            ></div>
            <div
              class="vui-load-more-text"
              :style="textStyle"
              v-show="!pull||['success','fail','nomore'].indexOf(status)!==-1"
            >{{text}}</div>
            <div
              class="vui-load-more-bg"
              :style="bgStyle"
              v-show="['success','fail','nomore'].indexOf(status)!==-1||!pull"
            ></div>
          </div>
        </slot>
      </div>
