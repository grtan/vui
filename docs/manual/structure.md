...
一个目录下只存放一个组件，约定给组件都设置name为`vui-xxx`格式，组件里的样式名都采用`vui-xxx-`前缀，且组件根元素都设置`vui-xxx`类名。css规则也必须遵循如上写法，限制只应用到组件内部元素上，防止影响页面上其他的元素。如果组件样式与皮肤无关，请将样式写在`<style>`标签内，否则写在`style`目录下对应的皮肤`less`文件中。有时在响应式页面中，当页面宽度变化时，有些组件需要支持`pc`和`mobile`样式切换。`vivo-ui`也支持组件样式切换。
